...
class RekapExport implements FromQuery, WithColumnFormatting, WithMapping, WithHeadings,ShouldAutoSize,WithStyles
{
    ...
    {
        $tanggal_mulai = $this->filters['mulai'] ?? null;
        $tanggal_akhir = $this->filters['akhir'] ?? null;
    ...
    {
        $jabatan_id = $this->filters['jabatan_id'] ?? null;

        return User::when($jabatan_id, function($query) use ($jabatan_id) {
